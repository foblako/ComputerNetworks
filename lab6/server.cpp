...
struct Client {
    ...
    size_t last_ids_pos;
    Client() : fd(-1), last_ids_pos(0) { memset(last_ids, 0, sizeof(last_ids)); }
};
...
// Параметры эмуляции
static std::atomic<int>    sim_delay_ms{0};
static std::atomic<double> sim_drop_rate{0.0};
static std::atomic<double> sim_corrupt_rate{0.0};
...
static int find_by_fd(int fd) {
    for (size_t i = 0; i < clients.size(); i++) {
        if (clients[i].fd == fd) return (int)i;
    }
    return -1;
}

// Проверка дубликата по списку last_ids; если новый — записывает в кольцевой буфер.
static bool is_dup_and_mark(int fd, uint32_t msg_id) {
    bool dup = false;
    pthread_mutex_lock(&cli_mu);
    int idx = find_by_fd(fd);
    if (idx >= 0) {
        for (size_t i = 0; i < DEDUP_SIZE; i++) {
            if (clients[idx].last_ids[i] == msg_id) { dup = true; break; }
        }
        if (!dup) {
            clients[idx].last_ids[clients[idx].last_ids_pos] = msg_id;
            clients[idx].last_ids_pos = (clients[idx].last_ids_pos + 1) % DEDUP_SIZE;
        }
    }
...
static void deliver_offline(const std::string &nick, int fd) {
    ...
    for (auto &o : mine) {
        ...
        pthread_mutex_lock(&hist_mu);
        for (auto &h : history) {
            if (h.msg_id == o.msg_id) { h.delivered = true; break; }
        }
        save_history_unlocked();
        pthread_mutex_unlock(&hist_mu);
    }
...
// Эмуляция помех: возвращает true если сообщение нужно отбросить
static bool simulate_impairment(MessageEx *m) {
    int d = sim_delay_ms.load();
    if (d > 0) {
        pthread_mutex_lock(&print_mu);
        std::cout << "[Transport][SIM] DELAY applied: " << d << " ms" << std::endl;
        pthread_mutex_unlock(&print_mu);
        usleep((useconds_t)d * 1000);
    }

    double drop = sim_drop_rate.load();
    if (drop > 0.0) {
        double r = (double)rand() / RAND_MAX;
        if (r < drop) {
            pthread_mutex_lock(&print_mu);
            std::cout << "[Transport][SIM] DROP (id=" << m->msg_id
                      << ", rate=" << drop << ")" << std::endl;
            pthread_mutex_unlock(&print_mu);
            return true;
        }
    ...
    double corrupt = sim_corrupt_rate.load();
    if (corrupt > 0.0 && m->length > 0) {
        double r = (double)rand() / RAND_MAX;
        if (r < corrupt) {
            uint32_t pos = (uint32_t)rand() % m->length;
            m->payload[pos] ^= 0xFF;
            pthread_mutex_lock(&print_mu);
            std::cout << "[Transport][SIM] CORRUPT payload (id=" << m->msg_id
                      << ", pos=" << pos << ")" << std::endl;
            pthread_mutex_unlock(&print_mu);
        }
    }
...
// Отправка ACK для msg_id
static int send_ack(int fd, uint32_t msg_id, const std::string &target_nick) {
    MessageEx ack;
    memset(&ack, 0, sizeof(ack));
    ack.type = MSG_ACK;
    ack.msg_id = msg_id; // ACK переносит исходный msg_id
    ack.timestamp = (int64_t)time(nullptr);
    strncpy(ack.sender, "server", MAX_NAME - 1);
    strncpy(ack.receiver, target_nick.c_str(), MAX_NAME - 1);
    ack.length = 0;
    pthread_mutex_lock(&print_mu);
    std::cout << "[Transport][ACK] send MSG_ACK (id=" << msg_id << ")" << std::endl;
    pthread_mutex_unlock(&print_mu);
    return send_msgex(fd, &ack);
}
...
static void handle_session(int fd) {
    ...
    // HELLO/WELCOME
    if (recv_msgex(fd, &msg) < 0 || msg.type != MSG_HELLO) { close(fd); return; }
    MessageEx wel;
    make_msg(&wel, MSG_WELCOME, "server", "", addrbuf);
    if (send_msgex(fd, &wel) < 0) { close(fd); return; }
    ...
    while (true) {
        ...
        if (recv_msgex(fd, &msg) < 0) { close(fd); return; }
    ...
    while (true) {
        memset(&msg, 0, sizeof(msg));
        if (recv_msgex(fd, &msg) < 0) { remove_client(fd); return; }
        uint32_t orig_id = msg.msg_id;

        log_recv_tcpip(msg.type, msgex_wire_size(&msg));

        // Применяем сетевые помехи (только для пользовательских типов)
        bool simulate = (msg.type == MSG_TEXT || msg.type == MSG_PRIVATE ||
                         msg.type == MSG_PING);
        if (simulate) {
            if (simulate_impairment(&msg)) {
                // Сообщение «потеряно» — никакой обработки и ACK
                continue;
            }
        }

        // Дедупликация (для типов, требующих ACK)
        bool dedup_relevant = (msg.type == MSG_TEXT || msg.type == MSG_PRIVATE);
        if (dedup_relevant) {
            if (is_dup_and_mark(fd, orig_id)) {
                pthread_mutex_lock(&print_mu);
                std::cout << "[Application][DEDUP] duplicate ignored (id=" << orig_id << ")" << std::endl;
                pthread_mutex_unlock(&print_mu);
                continue;
            }
        }
        ...
        if (msg.type == MSG_TEXT) {
            ...
            std::string line = "[" + fmt_time(ts) + "][id=" + std::to_string(mid) +
                               "][" + nick + "]: " + body;
            pthread_mutex_lock(&print_mu);
            std::cout << "[Application][ACK] process MSG_TEXT (id=" << orig_id << ")" << std::endl;
            std::cout << line << std::endl;
            pthread_mutex_unlock(&print_mu);

            HistEntry h;
            h.msg_id = mid; h.timestamp = ts; h.sender = nick; h.receiver = "";
            h.type = MSG_TEXT; h.text = body; h.delivered = true; h.is_offline = false;
            add_history(h);

            broadcast_text(nick, body, mid, ts, fd);
            send_ack(fd, orig_id, nick);
            ...
            pthread_mutex_lock(&print_mu);
            std::cout << "[Application][ACK] process MSG_PRIVATE (id=" << orig_id << ")" << std::endl;
            pthread_mutex_unlock(&print_mu);
            ...
            if (dst_fd < 0) {
                ...
                OfflineMsg om;
                om.msg_id = mid; om.timestamp = ts; om.sender = nick;
                om.receiver = target; om.text = body;
                pthread_mutex_lock(&off_mu);
                offline_queue[target].push_back(om);
                pthread_mutex_unlock(&off_mu);

                HistEntry h;
                h.msg_id = mid; h.timestamp = ts; h.sender = nick;
                h.receiver = target; h.type = MSG_PRIVATE; h.text = body;
                h.delivered = false; h.is_offline = true;
                add_history(h);
                ...
                MessageEx out;
                memset(&out, 0, sizeof(out));
                out.type = MSG_PRIVATE; out.msg_id = mid; out.timestamp = ts;
                ...
                h.msg_id = mid; h.timestamp = ts; h.sender = nick;
                h.receiver = target; h.type = MSG_PRIVATE; h.text = body;
                h.delivered = (rc == 0); h.is_offline = false;
                add_history(h);
            ...
            pthread_mutex_lock(&print_mu);
            std::cout << "[Transport][PING] recv MSG_PING (id=" << orig_id << ")" << std::endl;
            pthread_mutex_unlock(&print_mu);
            MessageEx out;
            memset(&out, 0, sizeof(out));
            out.type = MSG_PONG;
            out.msg_id = orig_id; // тот же msg_id
            out.timestamp = (int64_t)time(nullptr);
            strncpy(out.sender, "server", MAX_NAME - 1);
            strncpy(out.receiver, nick.c_str(), MAX_NAME - 1);
            pthread_mutex_lock(&print_mu);
            std::cout << "[Transport][PING] send MSG_PONG (id=" << orig_id << ")" << std::endl;
            pthread_mutex_unlock(&print_mu);
            if (send_msgex(fd, &out) < 0) { remove_client(fd); return; }
...
// Парсинг аргументов командной строки --delay=N --drop=R --corrupt=R
static void parse_args(int argc, char **argv) {
    for (int i = 1; i < argc; i++) {
        std::string a = argv[i];
        if (a.rfind("--delay=", 0) == 0) {
            sim_delay_ms.store(atoi(a.c_str() + 8));
        } else if (a.rfind("--drop=", 0) == 0) {
            sim_drop_rate.store(atof(a.c_str() + 7));
        } else if (a.rfind("--corrupt=", 0) == 0) {
            sim_corrupt_rate.store(atof(a.c_str() + 10));
        }
    }
}
...
int main(int argc, char **argv) {
    ...
    {
        std::ostringstream oss;
        oss << "[Application] sim params: delay=" << sim_delay_ms.load()
            << "ms drop=" << sim_drop_rate.load()
            << " corrupt=" << sim_corrupt_rate.load();
        print_line(oss.str());
    }
